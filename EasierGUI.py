                ...
                with gr.Row():
                    ...
                    with gr.Row():      
                        with gr.Column():
                            ...
                            with gr.Accordion(i18n("Time Calculator"), open=False):
                                with gr.Blocks() as demo:
                                    num_epochs_inicio_entry = gr.Number(label=i18n("Number of initial epochs:"), minimum=0, maximum=10000)
                                    num_epochs_fim_entry = gr.Number(label=i18n("Number of final epochs:"), minimum=0, maximum=10000)
                                    tempo_epoch_entry = gr.Number(label=i18n("Average time per epoch (seconds):"))
                                    with gr.Row():
                                        submit_btn = gr.Button(i18n("Calculate"))
                                    diagnosis_box1 = gr.Textbox(label=i18n("Result"))
    
                                    def calcular(num1, num2, num3):
                                        segundos_total = (num2 - num1) * num3
                                        minutos = int((segundos_total % 3600) // 60)  # 1 minute = 60 seconds
                                        horas = int(segundos_total // 3600)  # 1 hour = 3600 seconds
                                    ...
                                    # total_time = (num_epochs_end - num_epochs_beginning) * average_time_by_epoch
                                ...
                                with gr.Accordion(i18n("Minutes to Seconds"), open=False):
                                    with gr.Blocks() as demo:
                                        num_epochs_inicio_entry = gr.Number(label=i18n("Minutes"))
                                        with gr.Row():
                                            submit_btn = gr.Button(i18n("Calculate"))
                                        diagnosis_box2 = gr.Textbox(label=i18n("Result"))
    
                                        def calcularsec(minutos):
                                             mintosec = int(minutos * 60)
                                             return {
                                                 diagnosis_box2: i18n(f"{mintosec} seconds."),
                                                 }
                                        # total_time = (num_epochs_end - num_epochs_beginning) * average_time_by_epoch
